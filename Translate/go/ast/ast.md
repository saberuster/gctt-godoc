...
NewIdent 返回一个没有位置信息的 Ident。除了Go解析对代码生成 ASTs 也很有帮助。
...
IsExported 判断 id 是不是可导出的 Go 符号（首字母是否大写）。
...
Importer 处理包中的 import 路径。imports 以包 id （权威导入路径）为索引记录已经被导入的包。Importer 处理权威导入路径并检查它是否已经在 imports 中。如果在就返回 imports 中对应的实例，否则会根据导入路径加载对应的包，将记录保存在 imports 中并返回这条记录。
...
MergeMode 参数控制 MergePackageFiles 的行为。
...
Object 代表命名过的语言实体（包、常量、类型、变量、函数或标签）。
...
NewObj 返回指定名称和 ObjKind 的 Object。
...
Pos 返回 obj 声明的位置。如果计算位置时出错（nil，声明不正确）返回无效位置值。
...
Package 代表会被编译到同一个 Go 包内的源文件集合。
...
NewPackage 从一组 File 节点创建一个包节点。它会处理跨文件的标识符并修改每个文件的未处理列表。如果提供了 importer 和 universe 作用域那么它会处理不在任何包文件中声明的标识符。最后所有未处理的标识符都会被认为是未声明的。如文件属于不同的包，会选择一个包名并忽略不同于这个包名的文件，返回值是一个包节点如果发生错误也会返回 scanner.ErrorList。
...
Scope 维护在该作用域内的声明和其他作用域（outer）的外部链接。
...
Insert 会向 s 中插入一个指定名称的 obj，如果在 s 中没有该名称的 obj 会把 obj 插入进 s 并返回 nil。否则不改变 s 并返回 obj。
...
Lookup 返回 s 中名为 name 的 Object（不包括 Outer），如果没有返回 nil。
...
Spec 代表所有 *ImportSpec、*ValueSpec和*TypeSpec。
...
StarExpr 代表 "*" 格式的表达式。在语义上可以是一个 "*" 表达式或者指针类型。
...
TypeSwitchStmt 代表 type switch 语句。
...
UnaryExpr 代表一元表达式。StarExpr 代表 "*" 表达式。
...
Visitor 的 visit 方法会在 Walk 中应用在每个节点上。如果返回值 w 不是 nil，Walk 将会访问这个节点的子节点直到出现 w.Visit(nil)。
